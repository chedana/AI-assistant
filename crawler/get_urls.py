...
def _rightmove_tokenize_query(search_location: str) -> str:
    cleaned = re.sub(r"[^A-Za-z0-9]+", "", search_location or "").upper()
    parts = [cleaned[i : i + 2] for i in range(0, len(cleaned), 2) if cleaned[i : i + 2]]
    return "/".join(parts)


def _fetch_typeahead_candidates(search_location: str) -> list[dict]:
    tokenized = _rightmove_tokenize_query(search_location)
...
def _pick_best_typeahead_candidate(search_location: str, candidates: list[dict], prefer_region: bool = True) -> dict:
    if not candidates:
    ...
    for c in candidates:
        display = str(
            c.get("displayName") or c.get("name") or c.get("location") or c.get("text") or ""
        ).strip()
...
def resolve_with_typeahead(
    ...
    """
    Resolve a postcode/location string to Rightmove locationIdentifier
    using the typeahead API (no browser required).
    Returns a dict with location_identifier, display_location_identifier, etc.
    """
    candidates = _fetch_typeahead_candidates(search_location)
    chosen = _pick_best_typeahead_candidate(search_location, candidates, prefer_region=prefer_region)

    resolved_search_location = str(
        chosen.get("displayName") or chosen.get("name") or chosen.get("location") or chosen.get("text") or search_location
    ).strip()
